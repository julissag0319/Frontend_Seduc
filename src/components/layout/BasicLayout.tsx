  ...
  return (
    <div className="h-full flex justify-center items-center border-4 border-white rounded-lg ">
      <div className="max-w-1xl bg-white rounded-md p-4 ">
        <h2 className="pb-5 text-center font-bold text-xl text-black">{title}</h2>
        ...
        <div className="flex flex-wrap justify-center items-start h-full justify-items-center">
          ...
          <div
            ...
            <BsBookmarkCheck style={{ color: 'black', fontSize: '50px', margin: 'auto' }} />
            <br></br>
            <p className="font-bold text-black text-xl">{titleAdd}</p>
          </div>

          {titleList != 'Exportar a Excel' ?
            <HasRole rol={sessionStorage.getItem("NOMBRE_ROL")} arrRole={['Administrador', 'Nacional', 'Departamental', 'Distrital', 'Distrital', 'Redes']}>
              <div
                className="cards-menu-div hover:cursor-pointer border border-color-fondo rounded-lg"
                onClick={() => Navigate(navigateListTo)}>
                <BsFiles style={{ color: 'black', fontSize: '50px', margin: 'auto' }} />
                <br></br>
                <p className="font-bold text-black text-xl">{titleList}</p>
              </div>
            </HasRole>
            : <div
              className="cards-menu-div hover:cursor-pointer border border-color-fondo rounded-lg"
